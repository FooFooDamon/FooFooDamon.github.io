...
2. 创建自定义目录及文件：
    ...
    $ mkdir custom/root # 以下均是测试示例，根据实际情况而定，可创建多个目录及文件
    $ echo haha > custom/root/.bashrc
    ````

    更多细节详见[官方文档](https://buildroot.org/downloads/manual/manual.html#rootfs-custom)。
    若链接失效，可点击<a href="references/buildroot_manual.pdf">此处</a>
