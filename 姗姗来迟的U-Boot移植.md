...
* **最重要**别忘记修改**环境变量的分区偏移量和大小**：
    ...
    新代码的`uboot`分区只有`4M`，若`CONFIG_ENV_OFFSET`不同步改小，则在执行`saveenv`命令保存环境变量时，
...
不过最后的修改结果又很简单，仅需在`src/uboot/include/configs/mx6ullevk.h`增加和修改若干内容：
    ````
...
不会太复杂但也不会太简单，最重要的是能减少人力劳动。

* `Makefile`的详细逻辑不会在本文展开，但会在后续专门的文章进行解释，
链接则放在文末。
...
就是说会破坏根文件系统，后续重启会卡住。除了要做前述的修正，还要重新修复根文件系统。
由于前面已经移植好`USB`，所以可以将根文件系统镜像放到U盘里，
然后直接在`uboot`命令行进行烧写，过程如下（注意镜像后缀是`.ubi`而非`.ubifs`，
镜像生成方法可参考《[Buildroot及BusyBox深度排雷](https://foofoodamon.github.io/Buildroot及BusyBox深度排雷.html)》
...
除了可用厂商提供的工具烧到储存卡来进行整个系统的烧录，还可以在操作系统里单独烧`uboot`，
...
在`uboot`命令行下通过`tftp`命令下载文件时会出现以下现象：
...
本来这种情况比较难排查，但由于之前做内核移植时，曾遇过网络通但`SSH`操作很卡的情况，
...
这是上一个错误（即以太网口配置错误）触发的，报错现象类似下面这样：
...
于是引发了异常而重置。本来应该修改对应的业务代码，但若时间比较紧迫，或者要修改的范围太大，
则可以临时禁用这项检查，等以后有时间再慢慢改，或者等待修复这类缺陷的新版本`uboot`发布。
...
利己利他。本文所涉及的源码及脚本将在整理完毕后公开其项目链接。
