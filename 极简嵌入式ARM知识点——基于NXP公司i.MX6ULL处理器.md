...
* 接口电平：
    ...
    2. `RS-232`：即`COM口`（`DB9`接口，但通常只使用其中`3`条线，其余线仅以前用到），
    ...
    速度最高可达`10 Mbps`，但属于`半`双工），`+2`～`+6`V表示逻辑`0`，`-2`～`-6`V表示逻辑`1`，
...
-- | -- | -- | -- | -- | --
UART | GND<br>RXD<br>TXD | 一对一 | 典型：115200 Bd | 最简单 | 目标板调试、低速率工控设备通讯、蓝牙、GPS、GPRS等
